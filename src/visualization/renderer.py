"""
Main PyGame renderer for the audio visualizer.
"""
...
class Renderer:
    """
    Main PyGame renderer handling the display loop.

    Manages the window, event handling, and orchestrates
    the visualization drawing with multiple modes and effects.
    """
    ...
    def __init__(self, width: int = WINDOW_WIDTH, height: int = WINDOW_HEIGHT):
        ...
        # Create display with double buffering and hardware acceleration
        ...
        # Visualization manager (handles multiple modes with transitions)
        self.visualizer_manager = VisualizerManager(width, height)

        # Post-processing effects
        self.post_processor = PostProcessor(width, height)

        # Style transfer (optional, requires ONNX models)
    ...
    def update(self, features: AudioFeatures) -> None:
        """
        Update all visualizers with new audio features.
    ...
    def draw(self, features: AudioFeatures, source_name: str = "",
        ...
        """
        Draw the complete visualization frame.

        Args:
            features: Current audio features from analyzer
            source_name: Name of current audio source
        ...
        if self.style_transfer_enabled:
            ...
            if styled:
                # Blend styled frame with original
    ...
    def _draw_level_meters(self, features: AudioFeatures) -> None:
        """Draw bass/mid/treble level meters."""
        meter_width = 60
        meter_height = 8
    ...
    def quit(self) -> None:
        ...
        # Stop style transfer if running
        if self.style_transfer_enabled:
            self.style_transfer.stop()
        pygame.quit()
